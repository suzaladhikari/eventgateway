...
### Producer 
docker build --platform linux/amd64 -t eventgateway-producer . ## Building the docker image from the dockerfile
...
docker build --platform linux/amd64 -t eventgateway-worker .
